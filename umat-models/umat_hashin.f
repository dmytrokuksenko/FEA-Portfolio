...
!     Foundation, Inc., 675 Mass Ave, Cambridge, MA 02139, USA.
       ...
       SUBROUTINE umat_hashin(stre,xstate,ddsdde,sse,spd,scd,rpl,
...
!     
     ...
     &    g12, g13, g23, ntens)
            ...
            call hashin_failure(xt, xc, yt, yc, zc, zt, s13, s23,
     &    s12, stre, e, ntens)

            if ((maxval(e).GT.1.0).or.(minval(dmg).LT.1.0)) then      
                  
                  call hashin_failure_calc(stre, e, dmg, beta_fc, 
     ...
     &      g12, g13, g23, ntens)
      ...
      return
       ...
       subroutine hashin_failure(xt, xc, yt, yc, zc, zt, s13, s23,
     &    s12, stre, e, ntens)
...
!           Fiber tensile/compressive damage
            ...
            if (stre(1).GE.0.0) then
                e(1) = sqrt((stre(1)/xt)**2 + 
     &          (stre(4)**2 + stre(5)**2)/s12**2)
            else
                e(1) = sqrt((stre(1)/xc)**2)
            end if

!          Matrix tensile/compressive damage
                
            if ((stre(2)+stre(3)).GE.0.0) then
                e(2) = sqrt((stre(2)+stre(3))**2/yt**2
     &          + (stre(6)**2-stre(2)*stre(3))/s23**2
     &          + (stre(4)**2+stre(5)**2)/s12**2)
            else
                e(2) = sqrt(((yc/2*stre(6))**2)*
     &          (stre(2)+stre(3))/yc 
     &          + (stre(2)+stre(3))**2/(4*stre(6)**2)
     &          + (stre(6)**2 - stre(2)*stre(3))/stre(6)**2
     &          + (stre(4)**2+stre(5)**2)/stre(4)**2)  
            end if


!           Interlaminar damage

            if (stre(3).GE.0.0) then
                e(3) = (stre(3)/zt)**2
            else
                e(3) = (stre(3)/zc)**2
            end if
      ...
      return 
       ...
       subroutine hashin_failure_calc(stre, e, dmg, beta_fc, beta_ft,
...
!           Fiber tensile/compressive damage
     ...
     & kinc 
                  ...
                  dmg(4) = dmg(4)*(0.99999-beta_s)
                  write(1, *) 'Hashin damage coef:', e(1)
                  write(1, *) 'Updated damage coef 1:', dmg(1)
                  write(1, *) 'Updated damage coef 4:', dmg(4)
...
!           Matrix tensile/compressive damage
     ...
     & kinc
                  ...
                  dmg(4) = dmg(4)*(0.99999-beta_s)
                  dmg(6) = dmg(6)*(0.99999-beta_s)
                  write(1, *) 'Hashin damage coef:', e(2)
                  write(1, *) 'Updated damage coef-1:', dmg(2)
                  write(1, *) 'Updated damage coef-4:', dmg(4)
                  write(1, *) 'Updated damage coef-6:', dmg(6)
...
!           Interlaminar tensile/compressive damage
     ...
     &    'interlaminar tensile damage detected at ', kinc
                  ...
                  write(1, *) 'Hashin damage coef:', e(3)
